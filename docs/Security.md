...
- Admin sessions use signed HS256 JWTs with a seven-day expiry, fixed issuer, and fixed audience.
...
- Tokens issued before issuer/audience hardening remain accepted only when both claims are absent and only until their normal expiry. New tokens always include both claims.
...
- Unsafe API requests with a browser `Origin` header are same-origin only. `NEXT_PUBLIC_BASE_URL` may define the canonical deployment origin. No permissive CORS headers are emitted.
...
- Upload middleware rejects oversized request bodies early; the storage layer still performs authoritative size, category, MIME, extension, signature, and path validation.

## Response security

All application responses disable MIME sniffing and framing, restrict referrer information and browser permissions, and apply CSP restrictions for frames, forms, base URLs, and object embeds. Production responses also emit HSTS. The `X-Powered-By` header is disabled.
...
`npm run release:verify` is fail-closed: it runs TypeScript, zero-warning lint, automated tests, the production build, the storage migration dry run, and `npm audit --omit=dev`. As of 2026-08-06, the entire command passes and the production audit reports zero vulnerabilities.
