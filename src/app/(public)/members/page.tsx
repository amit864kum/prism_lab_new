...
export default async function MembersPage() {
  const members = await getPublicMembers(
    { statuses: ['current', 'completed', 'alumni'] },
    { sort: { status: 1, role: 1, displayOrder: 1, yearJoined: -1, name: 1 } }
  )

  const serializedMembers = members.map(toMemberListViewModel)
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 pb-20">
      <div className="relative bg-slate-900 text-white overflow-hidden py-16 sm:py-20 border-b border-slate-800">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight leading-none">Lab Members</h1>
          <p className="text-sm sm:text-base text-slate-405 max-w-xl font-medium">
            Meet current and completed members of PRISM Lab, grouped by academic role.
          </p>
        </div>
      </div>
