...
import CurrentMembersClient from '@/components/public/CurrentMembersClient'
...
export default async function CurrentMembersPage() {
  ...
  const currentMembers = await Member.find({ status: 'current' })
    .sort({ yearJoined: -1, name: 1 })
    .lean()

  // Serialize ObjectIds and dates for client component safety
  const serializedMembers = currentMembers.map((m: any) => ({
    _id: m._id.toString(),
    name: m.name,
    slug: m.slug,
    role: m.role,
    status: m.status,
    yearJoined: m.yearJoined || null,
    imageUrl: m.imageUrl || '',
    bio: m.bio || '',
    email: m.email || '',
    linkedinUrl: m.linkedinUrl || '',
    googleScholarUrl: m.googleScholarUrl || '',
    publications: m.publications ? m.publications.map((p: any) => p.toString()) : [],
  }))
