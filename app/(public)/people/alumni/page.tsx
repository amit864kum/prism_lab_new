...
import { Mail, GraduationCap, User, Calendar, ExternalLink } from 'lucide-react'
...
export default async function AlumniPage() {
  ...
  const alumni = await Member.find({ status: 'alumni' })
    .sort({ yearLeft: -1, name: 1 })
    .lean()
                ...
                {group.members.map((member) => (
                  ...
                  >
                      ...
                      <div className="space-y-1 w-full">
                        ...
                        {member.yearJoined && (
                          <p className="text-[10px] font-semibold text-slate-450 dark:text-slate-500 tracking-tight mt-0.5">
                            {member.yearJoined} - {member.yearLeft || 'Present'}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="w-full mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3 z-10">
                      ...
                      <Link
                        href={`/people/current-members/${member._id.toString()}`}
